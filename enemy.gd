...
func _physics_process(delta: float) -> void:
	...
	var cloud_half_h: float = 14.0 if ptype == 3 else 4.0
	...
	if speed > 0.0:
		...
		var cloud_left: float = cloud_ref.global_position.x - 50.0
		var cloud_right: float = cloud_ref.global_position.x + 50.0
